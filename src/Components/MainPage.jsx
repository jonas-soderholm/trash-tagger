...
function MainPage() {
  ...
  function CreateNewMapClick() {
    ...
    if (mapArray.length < 5 && inputValue !== "") {
      ...
      setMapArray((prevMapArray) => [inputValue, ...prevMapArray]);
      setInputValue("");
    } else {
    ...
    return (
      <>
        <div className="Header bg-[#cd3a3a00] text-2xl pt-10 px-10 text-slate-200 text-center">
          {mapArray.length !== 0 ? `Current area: ${currentMap}` : <span style={{ visibility: "hidden" }}>Hidden</span>}
        </div>
    ...
    return (
      <div
        className="button flex justify-between hover:bg-slate-500 cursor-pointer gap-2
         m-4 p-3 bg-[#888888] rounded-full"
    ...
    <>
      <div className="main-container md:flex md:flex-row  bg-[#46664200] m-[2vh] rounded-lg h-[96vh] gap-4">
        <div className="left-part md:w-1/3 bg-slate-700 rounded-lg">
          ...
          <div className="input flex items-center justify-center pt-10 gap-4">
            <input
              maxLength={50}
              type="text"
